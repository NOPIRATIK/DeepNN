...
# создадим 2 сумматора без функци активации, это называется полносвязный слой (fully connected layer)
...
# Задание 1
# 1
x = torch.randint(0, 100, (1,), dtype=torch.int64)
print(x)
# 2
x_float = x.to(dtype=torch.float32)
print(x_float)
# 3
x_tens = x_float.clone()
...
print(x_new)
# 3.2
random = torch.randint(1, 11, (1,), dtype=torch.float32)
x_new = x_new * random
print(x_new)
# 3.3
exp = torch.exp(x_new)
print(exp)
# 4
exp.backward(retain_graph=True)
print(x_tens.grad)
...
y_int = np.array([np.where(unique_classes == cls)[0][0] for cls in y])
...
# 3. Модель (как в примере, но 4 входа и 3 выхода)
linear = nn.Linear(4, 3)  # как nn.Linear(3, 2) в примере
