...
# смотрим что в них
print(df.head()) #значение внутри функции дает возможность вывести большее кол-во строк
...
X = df.iloc[:,[0,1,2]].values #БЕРЕМ ТРИ ПРИЗНАКА(СТОЛБЦА) И ВСЕ СТРОКИ
...
def neuron(w,x):
    if((w[1]*x[0]+w[2]*x[1]+w[3]*x[2]+w[0])>=0): # ДОБАВИЛИ 3 ПРИЗНАК
        predict = 1
    else: 
        predict = -1
    return predict
...
w = np.array([0, 0.1, 0.2, 0.4]) #ТЕПЕРЬ 4 ВЕСА
...
w = np.random.random(4) #ТЕПЕРЬ 4 ВЕСА ДЛЯ 3 ПРИЗНАКОВ
print(w)
...
fixed_feature = np.mean(X[:,1])
for i,w in zip(range(len(w_iter)), w_iter):
    yl = -(xl*w[1]+w[0]+w[3])*fixed_feature/w[2] # уравнение линии
