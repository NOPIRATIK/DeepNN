...
df = pd.read_csv('C:/Users/lexag/.spyder-py3/data.csv')
X = torch.Tensor(df.iloc[0:100, 0:3].values.astype(np.float32))
y = df.iloc[0:100, 4:5].values.flatten()
y = torch.Tensor(np.where(y == "Iris-setosa", 1, -1).reshape(-1,1).astype(np.float32))
...
df = pd.read_csv('C:/Users/lexag/.spyder-py3/data_3class.csv')
X = torch.Tensor(df.iloc[:, 0:3].values.astype(np.float32))
y = df.iloc[:, 4:5].values.flatten()         # ответы берем из четвертого столбца как и раньше
...
labels = pd.get_dummies(y).columns.to_numpy()  #  отдельно сохраним названия классов
y = torch.Tensor(pd.get_dummies(y).to_numpy()) # сохраним значения
...
df = pd.read_csv('C:/Users/lexag/.spyder-py3/dataset_simple.csv')

X = torch.Tensor(df['age'].values.reshape(-1, 1).astype(np.float32))
y = torch.Tensor(df['income'].values.reshape(-1, 1).astype(np.float32))
...
class IncomePredictor(nn.Module):
    def __init__(self, in_size, hidden_size, out_size):
        ...
        self.layers = nn.Sequential(
            nn.Linear(in_size, hidden_size),   # входной слой
            nn.Tanh(),                          # функция активации
            nn.Linear(hidden_size, hidden_size),
            nn.Tanh(),
            nn.Linear(hidden_size, out_size)    # выходной слой 
        )
    
    def forward(self, X):
        return self.layers(X)

inputSize = 1       # признак - возраст
hiddenSizes = 10    # 10 нейронов в скрытом слое
outputSize = 1      # предсказываем доход

net = IncomePredictor(inputSize, hiddenSizes, outputSize)
...
for i in range(epochs):
    pred_norm = net.forward(X_norm)
    loss = lossFn(pred_norm, y_norm)
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
    ...
    if i % 50 == 0:
        with torch.no_grad():
            ...
            mae = torch.mean(torch.abs(pred_orig - y))
        print(f'Эпоха {i:3d}: Loss = {loss.item():.6f}, MAE = {mae.item():.0f} руб.')

with torch.no_grad():
    predictions_norm = net.forward(X_norm)
...
print(f"(MAE): {mae.item():.0f} руб.")

print("-" * 65)
print("  Возраст | Реальный доход  \ Предсказанный  | Ошибка")
...
for i in range(min(15, len(X))):
    ...
    error = abs(real - pred)
    print(f"    {age:2.0f}    |     {real:6.0f}      |     {pred:6.0f}     |  {error:5.0f}")

def predict_income(age):
    with torch.no_grad():
        age_norm_input = (torch.Tensor([[age]]) - X_mean) / X_std
        pred_norm = net.forward(age_norm_input)
        pred = pred_norm * y_std + y_mean
        return pred.item()

ages_unique = sorted(df['age'].unique())
print("\nСредний доход по возрастам:")
for age in ages_unique[::3]:  # каждый третий возраст для краткости
    avg_income = df[df['age'] == age]['income'].mean()
    pred_income = predict_income(age)
    print(f"  Возраст {age:2d}: реальный средний = {avg_income:6.0f} руб., предсказанный = {pred_income:6.0f} руб.")
